...
import { CardNotArchivedError } from '../../domain/errors/card-not-archived.error';
import {
  LIST_REPOSITORY,
  type ListRepository,
...
@Injectable()
export class DeleteCardUseCase implements UseCase<DeleteCardCommand, void> {
  constructor(
  ...
  async execute(command: DeleteCardCommand): Promise<void> {
    ...
    if (card.status !== 'archived') {
      throw new CardNotArchivedError();
    }

    await this.cards.deleteCard(command.cardId);
  }
}
