...
interface UnarchiveListCommand {
  listId: string;
  currentUserId: string;
}

@Injectable()
export class UnarchiveListUseCase implements UseCase<UnarchiveListCommand, List> {
  constructor(
    @Inject(LIST_REPOSITORY) private readonly lists: ListRepository,
    @Inject(BOARD_REPOSITORY) private readonly boards: BoardRepository,
  ) {}

  async execute(command: UnarchiveListCommand): Promise<List> {
    ...
    return this.lists.update(list.unarchive());
  }
}
